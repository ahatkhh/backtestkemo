...
class Kemo_Strategy(Strategy):
    ...
    def __init__(self, isRealTime=False):
        ...
        self.state = Kemo_Strategy.STATE_NONE
    ...
    def next(self):
        ...
        # rev zone
        elif self.state == Kemo_Strategy.STATE_REV or \
                (self.state == Kemo_Strategy.STATE_NONE and self.broker.price < self.initialPrice):
            # above average
            if self.broker.price > self.rev_average:
                self.rev_trailHighest = max(self.rev_trailHighest, self.broker.price)
                # hands off
                if self.rev_average * (1 + self.rev_aboveAverage) < self.broker.price < self.rev_trailHighest * (1 - self.pro_trail):
                    self.close()
                    self.initialPrice = 0
                    self._handleEvent(
            ...
            # in sell position
                ...
                if self.broker.price < self.rev_intervals[-1]:
                    ...
                    self.state = Kemo_Strategy.STATE_REV
                    self._handleEvent(
            ...
            # in rebuy position
                ...
                if self.rev_trailLowest < self.broker.price < self.rev_intervals[-1] * (1 - self.rev_rebuyTrigP):
                    if self.broker.price > self.rev_trailLowest * (1 + self.rev_trail):      # rebuy
        ...
        # profit zone
            ...
            while intervalNum < self.pro_sellNum and\
                    self.pro_trailHighest > self.broker.price > self.pro_intervals[intervalNum+1] * self.initialPrice:
                intervalNum += 1

            if intervalNum > self.pro_intervalNum and self.broker.price < self.pro_trailHighest * (1 - self.pro_trail):
                self.sell(self.pro_sellP * self.initialBought * (intervalNum - self.pro_intervalNum))
            ...
            while intervalNum > 0 and\
                    self.pro_trailLowest < self.broker.price < self.pro_intervals[intervalNum-1] * self.initialPrice:
                intervalNum -= 1

            if intervalNum < self.pro_intervalNum and self.broker.price > self.pro_trailLowest * (1 + self.pro_trail):
                self.buy(self.pro_sellP * self.initialBought * (self.pro_intervalNum - intervalNum))
                self.pro_intervalNum = intervalNum
    ...
    def end(self):
        ...
        # self.plots.append({"values": df["price"], "scatter": True,
        #                    "x": df.index, "params": {"color": "blue"}})
